...
void Transform::SetZIndex(unsigned int newIndex)
{
    ...
    if (parentEntity != nullptr && parentEntity->parentScene != nullptr) {
        ...
        else if (newIndex > scene->highestZIndex)
            // set the scene's highest to be the new one
            scene->highestZIndex = newIndex;
...
glm::mat4 Transform::ToMatrix()
{
    ...
    if (parentEntity != nullptr && parentEntity->parentScene != nullptr) 
        ...
        transMatrix = glm::translate(transMatrix, glm::vec3((position.x*2.0f) + size.x, (position.y*2.0f) + size.y,-((float) parentEntity->parentScene->highestZIndex - _zIndex)));
    else
