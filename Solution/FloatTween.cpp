...
FloatTween::FloatTween(float* valueToTween, float startValue, float targetValue, double tweenDuration, double tweenDelay, bool deleteOnEnd, Method method)
{
	useSetter = false;
...
FloatTween::FloatTween(SetterCallback setterCallback, float startValue, float targetValue, double tweenDuration, double tweenDelay, bool deleteOnEnd, Method method)
{
	useSetter = true;
...
void FloatTween::Update(double deltaTime)
{
	...
	if (_progressedSeconds > _delay) {
		...
		float newValue = _startValue + (float)((_targetValue - _startValue) * (timeSinceStart / _duration));
