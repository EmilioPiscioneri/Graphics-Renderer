...
Vec3Tween::Vec3Tween(glm::vec3* valueToTween, glm::vec3 startValue, glm::vec3 targetValue, double tweenDuration, double tweenDelay, bool deleteOnEnd, Method method)
{
	useSetter = false;
...
Vec3Tween::Vec3Tween(SetterCallback setterCallback, glm::vec3 startValue, glm::vec3 targetValue, double tweenDuration, double tweenDelay, bool deleteOnEnd, Method method)
{
	useSetter = true;
...
void Vec3Tween::Update(double deltaTime)
{
	...
	if (_progressedSeconds > _delay) {
		...
		glm::vec3 newValue = _startValue + (_targetValue - _startValue) * (float)(timeSinceStart / _duration);
